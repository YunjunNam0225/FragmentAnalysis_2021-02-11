...
static void _CopyTex(float *dBuf, unsigned int dHeight, char dest, _TEXTUREA &ta, _TEXTUREB &tb, unsigned int yOff,
    unsigned int xOff, unsigned int yCount, unsigned int xCount, const char *desc);

static void _UpdateTex(_TEXTUREA &ta, _TEXTUREB &tb, unsigned int yOff, unsigned int xOff, const float *sBuf,
