...
% The following constant defines the size of several arrays that are members of a structure that is passed as a kernel
% argument.  It used to have minimum value 1.  It now has minimum value 2 because of what I assume is a compiler bug in
% CUDA 3.0.  The structure's contents were not being passed correctly.
printf('#define _NUM_CV_NZ %u\n', max(def.cvCount, 2));

WriteTexRefs('_TEXTUREA', '_g_tTDataA', def.tCount , '_GetTTexA');
WriteTexRefs('_TEXTUREB', '_g_tTDataB', def.tCount , '_GetTTexB');
...
for i = 1 : numel(def.types)
    ...
    for j = 1 : numel(d.cat.nc.svSyms)
        ...
        if ~d.sym.(name).private
            ...
            if ~ismember(key, rsList)
                WriteNTrans(d, platform, '_NTrans', d.sym.(name).pos, resNo, 'n');
                rsList{end + 1} = key;
            end
        end
    end
    for j = 1 : numel(d.cat.nw.svSyms)
        ...
        if ~ismember(key, rsList)
            WriteNTrans(d, platform, '_WTrans', d.sym.(name).pos, resNo, 'w');
            rsList{end + 1} = key;
        end
    end
end
...
function WriteNTrans(d, platform, fname, pos, resNo, type)

fn = sprintf('%s%us%u', fname, resNo - 1, d.dimNo - 1);

cns_transgen('handle', d, @printf, platform, fn, type);
cns_transgen('size'  , d, @printf, platform, fn, true);
...
WriteFields(q, cat, 's', 'READ_%N(...)'                 , true , '_Get%1%2(%R,%M,%N,0,%A)'  , va{:});
WriteFields(q, cat, 'm', 'READ_%N(E,...)'               , true , '_Get%1%2(%R,%M,%N,E,%A)'  , va{:});
...
WriteFields(q, cat, '' , 'READ_%N_HANDLE(H,...)'        , true , '_GetH%2(%R,H,%A)'         , va{:});
WriteFields(q, cat, '' , 'GET_%N_HANDLE_IPOS(H,...,Y,X)', false, '_GetH%21(%R,H,%A)'        , va{:});
...
if strcmp(varargin{2}, 'Array')
    ...
    WriteFields(q, cat, 's', 'GET_%N_IPOS(...,Y,X)'  , false, '_Get%1%21(%R,%M,%N,0,%A)', va{:});
    WriteFields(q, cat, 'm', 'GET_%N_IPOS(E,...,Y,X)', false, '_Get%1%21(%R,%M,%N,E,%A)', va{:});
...
WriteFields(q, cat, 'sp', 'READ_%T_%N(Z,...)'            , true , '_GetZ%1%2(%Z,%R,%M,%N,0,%A)'  , va{:});
WriteFields(q, cat, 'mp', 'READ_%T_%N(Z,E,...)'          , true , '_GetZ%1%2(%Z,%R,%M,%N,E,%A)'  , va{:});
...
WriteFields(q, cat, 'p' , 'READ_%H_HANDLE(H,...)'        , true , '_GetH%2(%R,H,%A)'             , va{:});
WriteFields(q, cat, 'p' , 'GET_%H_HANDLE_IPOS(H,...,Y,X)', false, '_GetH%21(%R,H,%A)'            , va{:});
...
if strcmp(varargin{2}, 'Array')
    ...
    WriteFields(q, cat, 'sp', 'GET_%T_%N_IPOS(Z,...,Y,X)'  , false, '_GetZ%1%21(%Z,%R,%M,%N,0,%A)', va{:});
    WriteFields(q, cat, 'mp', 'GET_%T_%N_IPOS(Z,E,...,Y,X)', false, '_GetZ%1%21(%Z,%R,%M,%N,E,%A)', va{:});
...
WriteFields(q, cat, '', 'READ_%T_%N(Z,...)'            , true , '_GetZC%1(%Z,%B,%F,%A)' , va{:});
WriteFields(q, cat, '', 'GET_%T_%N_HANDLE(Z)'          , false, '_GetZC%1H(%Z,%B,%F)'   , va{:});
WriteFields(q, cat, '', 'GET_%T_%N_IPOS(Z,...,Y,X)'    , false, '_GetZC%11(%Z,%B,%F,%A)', va{:});
WriteFields(q, cat, '', '%I_HANDLE'                    , false, '_DefC%1H(%B)'          , va{:});
WriteFields(q, cat, '', '%I_HANDLE_%C_SIZE(H)'         , false, '_GetHC%1Sz(H,%B,%D)'   , va{:});
WriteFields(q, cat, '', 'READ_%I_HANDLE(H,...)'        , true , '_GetHC%1(H,%B,%A)'     , va{:});
WriteFields(q, cat, '', 'GET_%I_HANDLE_IPOS(H,...,Y,X)', false, '_GetHC%11(H,%B,%A)'    , va{:});
...
WriteFields(q, cat, 'sp', 'READ_%T_%N(Z,...)'            , true , '_GetZ%1Field(%Z,%B,%A)' , va{:});
...
WriteFields(q, cat, 'sp', 'READ_%I_HANDLE(H,...)'        , true , '_GetH%1Field(H,%B,%A)'  , va{:});
WriteFields(q, cat, 'sp', 'GET_%I_HANDLE_IPOS(H,...,Y,X)', false, '_GetH%1Field1(H,%B,%A)' , va{:});
