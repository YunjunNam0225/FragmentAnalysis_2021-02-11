...
fprintf('Building the model on the %s....\n', upper(platform));
...
fprintf('Retrieving and plotting output....\n');

out = cns('get', 3, 'val');

figure;
subplot(2, 3, 1); imshow(shiftdim(in, 1));
subplot(2, 3, 2); imagesc(shiftdim(out(1, :, :), 1));
subplot(2, 3, 3); imagesc(shiftdim(out(2, :, :), 1));
subplot(2, 3, 5); imagesc(shiftdim(out(3, :, :), 1));
subplot(2, 3, 6); imagesc(shiftdim(out(4, :, :), 1));
