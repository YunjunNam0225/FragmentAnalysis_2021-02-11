...
%***********************************************************************************************************************
%     fprintf('%d-th layer (%s, %s) and %d-th way\n',z, m.layers{z}.type,method,dimID);
...
%     fprintf(1,'     nSize[%d] = 2 * floor((baseSize[%d] - factor[%d]) / (2 * factor[%d])) + 1\n',nSize,baseSize,factor,factor);
...
%         fprintf(1,'     nSize[%d] = 2 * floor((baseSize[%d]) / (2 * factor[%d]))\n',nSize,baseSize,factor);      
...
if mod(pSize, 2) == mod(rfSize, 2)
    ...
    if mod(rfStep, 2) == 0
        ...
        if isequal(parity, 1) || (isempty(parity) && (nSize1 >= nSize0))
            nSize = nSize1;
%             fprintf(1,'  a   nSize [%d] = 2 * floor((pSize[%d] - rfSize[%d]) / (2 * rfStep[%d])) + 1 [parity: %d]\n',nSize,pSize,rfSize,rfStep,parity);
        else
            nSize = nSize0;
%             fprintf(1,'  b   nSize [%d] = 2 * floor((pSize[%d] - rfSize[%d] - rfStep[%d]) / (2 * rfStep[%d])) + 2 [parity: %d]\n',nSize,pSize,rfSize,rfStep,rfStep,parity);
        ...
        nSize = nSize1;
%         fprintf(1,'  c   nSize [%d] = 2 * floor((pSize[%d] - rfSize[%d]) / (2 * rfStep[%d])) + 1 [parity: %d]\n',nSize,pSize,rfSize,rfStep,parity);
    ...
    nSize = nSize0;
%     fprintf(1,'  d   nSize [%d] = 2 * floor((pSize[%d] - rfSize[%d] - rfStep[%d]) / (2 * rfStep[%d])) + 2 [parity: %d]\n',nSize,pSize,rfSize,rfStep,rfStep,parity);
