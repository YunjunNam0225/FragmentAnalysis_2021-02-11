...
int   yWidth  = 1;//(yFactor < 1) ? (int)ceilf(4.0f / yFactor) : 4;
int   xWidth  = 1;//(xFactor < 1) ? (int)ceilf(4.0f / xFactor) : 4;
...
//WRITE_VAL( (float)xWidth );
//WRITE_VAL(fminf(fmaxf(num / den, 0.0f), 1.0f));
WRITE_VAL( READ_VAL_HANDLE(h, 0, 0, y1, x1) );
//WRITE_VAL( (float)y1 );
