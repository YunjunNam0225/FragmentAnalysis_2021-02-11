function varargout = fhpkg_cns_type_sdm(method, varargin)

% Defines the "sdm" cell type.  An "sdm" cell computes the response of a patch of cells in the previous layer to a stored
% (learned) template of the same size.  The "d" stands for "dense" -- the stored templates contain values for every
% feature at every position. The "m" stands for "mask".
%
...
% Holds the stored values that make up the mask.
d.mVals = {'ga', 'private', 'cache', 'dims', {1 2}, 'dparts', {1 1}, ...
    'dnames', {'y' 'x'}};
...
% Add "sdm" group G, defined by parameters P and dictionary D, to a network model.  Called from fhpkg_model via the
...
%
%    P.rfStep - distance between "sdm" cell centers, in number of previous layer units.  Commonly 1.
...
if isempty(d.mVals)
    d.mVals = ones(p.rfCountMin, p.rfCountMin);
end
